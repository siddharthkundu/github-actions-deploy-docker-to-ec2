...
POSTGRES_CLUSTER_ARN=${aws_rds_cluster.aurora.arn}

# The RDS Cluster Identifier
POSTGRES_CLUSTER_ID=${aws_rds_cluster.aurora.id}

# The RDS Cluster Resource ID
POSTGRES_CLUSTER_RESOURCE_ID=${aws_rds_cluster.aurora.cluster_resource_id}

# Writer endpoint for the cluster
POSTGRES_CLUSTER_ENDPOINT=${aws_rds_cluster.aurora.endpoint}

# A read-only endpoint for the cluster, automatically load-balanced across replicas
POSTGRES_CLUSTER_READER_ENDPOINT=${aws_rds_cluster.aurora.reader_endpoint}

# The running version of the cluster database
POSTGRES_CLUSTER_ENGINE_VERSION_ACTUAL=${aws_rds_cluster.aurora.engine_version_actual}

# Name for an automatically created database on cluster creation
# database_name is not set on `aws_rds_cluster` resource if it was not specified, so can't be used in output
POSTGRES_CLUSTER_DATABASE_NAME=${aws_rds_cluster.aurora.database_name == null ? "" : aws_rds_cluster.aurora.database_name}

# The database port
POSTGRES_CLUSTER_PORT=${aws_rds_cluster.aurora.port}
...
POSTGRES_CLUSTER_HOSTED_ZONE_ID=${aws_rds_cluster.aurora.hosted_zone_id}
